...
def detectaVerificacaoTestes(caminhoPrintTeste):
    ...
    cv2.imwrite("./src/Cortes" + "/Data" + ".jpg", data)
    cv2.imwrite("./src/Cortes" + "/Rec_processados" + ".jpg", rec_proc)

    def recorte(display):  # display = Data ou Rec_processados
        i = 0
        im = cv2.imread("./src/Cortes/" + str(display) + ".jpg")
        ...
        for c in cnts:
            ...
            if area > 10:
                if h > 10 and w < 10:
                    num = im[y : y + h, x : x + w]
                    cv2.imwrite("./src/numeros/" + str(display) + "/numero_" + str(i) + ".jpg", num)
                    i += 1
                elif h > 8 and w > 13 and w < 16:
                    metade = int(x + w / 2)
                    num1 = im[y - 1 : y + h + 1, x - 1 : metade]
                    num2 = im[y - 1 : y + h + 1, metade : x + w + 1]
                    cv2.imwrite("./src/numeros/" + str(display) + "/numero_" + str(i) + ".jpg", num1)
                    i += 1
                    cv2.imwrite("./src/numeros/" + str(display) + "/numero_" + str(i) + ".jpg", num2)
                    i += 1

    # Excluindo os números que já tinham sido salvos em outras compilações:
    data = list(paths.list_images("./src/numeros/Data"))
    rec_proc = list(paths.list_images("./src/numeros/Rec_processados"))
